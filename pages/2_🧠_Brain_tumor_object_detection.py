...
def load_all_models():
    ...
    model_ax = torch.hub.load(yolov5_path, 'custom', path=weights['Axial'], force_reload=True, source='local', device='cpu')
    model_ax.eval()
    models['Axial'] = model_ax

    model_cor = torch.hub.load(yolov5_path, 'custom', path=weights['Coronal'], force_reload=True, source='local', device='cpu')
    model_cor.eval()
    models['Coronal'] = model_cor

    model_sag = torch.hub.load(yolov5_path, 'custom', path=weights['Sagittal'], force_reload=True, source='local', device='cpu')
    model_sag.eval()
    models['Sagittal'] = model_sag
...
st.title('Определение опухолей с помощью моделей YOLO')
